...
| X | 自分のポスト + 自分のリプライ + いいね（本文込み） | フィード・フォロワー分析・DM・**ブックマーク（2026-07-15 実 export 検証: 公式アーカイブに含まれず対象外確定）** |
...
1. ~~**X ブックマークの取得可否**~~ → **解消（2026-07-15、実 export で確認）**。
   実 X アーカイブ（7GB、`manifest.js` の `dataTypes` に bookmark 系エントリが
   存在しない）を検査した結果、**X の公式アーカイブ生成ツール自体が
   bookmark という dataType を認識していない**——ブックマークのエクスポート
   機能が無い。よって「アーカイブにあれば取り込む、無ければ対象外」の
   条件により **X ブックマークは対象外で確定**。X 側の取り込み対象は
   「自作投稿＋リプライ」と「いいね」の 2 種のみとなる（実データ:
   tweets 123,223件・likes 82,482件、bookmarks 0件。likes は `like.js` の
   `fullText` で本文込み取得を確認、`external_id` 205,705件重複ゼロ）。
...
- 実 X アーカイブ 1 件・実 FB DYI 1 件を取り込み、自作は `social_post`、
  いいねは `bookmark(social meta)` として索引される（X ブックマークは
