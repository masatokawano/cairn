...
- **発見の要約行（v1.4 追加、2026-07-15 オーナー指示）**: 発見（Karakeep）の各行の下に、
  Karakeep の AI 要約 `summary`（無ければページ説明 `description` にフォールバック）を
  1 行（エスケープ + 切り詰め）で表示する。ブックマーク先を開かなくても内容が分かる
  ようにするため（S1 に寄与）。要約の生成は Karakeep 側の自動要約
  （`INFERENCE_ENABLE_AUTO_SUMMARIZATION`、ローカル ollama）で行い、Cairn からは
  読み取るだけ（不変条件どおり connectors は read-only）。
