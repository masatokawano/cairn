...
- **依頼された範囲そのものを納品する**: 黙って狭める・広げる・別物に作り替えない。
  解釈が割れる箇所だけ確認し、定型の判断は自分で決めて進める。依頼が誤っていると
  思ったら 1 文で指摘したうえで依頼どおり進める。**タスクは最後まで終える**。
  一部が実施不能なら残りを全部やり切り、何を残したか・なぜかを明示する
  （縮小の判断はユーザーのもの）。
...
- 作業途中の思考メモは簡潔でよいが、**最終報告はこのセッションを見ていない読者が
  読んで分かる完全な文**で書く。
- **出力長をタスクに合わせる**: 会話の応答も、ディスクに書く成果物（報告・設計メモ・
  markdown）も、必要な実質を尽くしたら止める。冗長な要約の重複・定型の前置き・
  埋め草の節を足さない。短くする手段は「含める項目を絞る」ことであって、
  文を断片や矢印記法に圧縮することではない。
- **作業中の実況は要点だけ**: 最初のツール呼び出しの前に何をするかを 1 文。作業中は
  重要な発見か方針変更があったときだけ短く。終了時は結論（何が起きたか / 何が分かったか）
  を最初の 1 文に置き、詳細はその後に置く。ルーチンな操作を逐一ナレーションしない。
- **自己訂正は結果が変わるときだけ書く**: ユーザーのコード・結論・判断が変わる誤りだけを
  簡潔に訂正して作業を続ける。結果が変わらない言い間違いは黙って直す。謝罪・自己批判・
  過去の誤りの列挙はしない（思考ブロック内には適用しない）。ユーザーからの追加質問は
  それ自体が「間違えた」の合図ではない。正しかった記述を再監査しない。
- サブエージェントへの委任は read-only の調査・テスト実行・ログ解析に限る。
  DB・migration・deliver 系への書き込みを伴う変更は主エージェントが自ら行う。
- **委任は控えめに**: サブエージェントは文脈の再確立・報告の読み直しでコストと時間が
  倍増する。自分が数回のツール呼び出しで終えられる作業は委任しない。**検証・
  ダブルチェック目的で委任しない**（検証は主エージェントのループで行う）。1 本で
  足りるなら 1 本にし、並列に投げるのは互いに独立した大きめの調査に限る。
...
| 役者 | 既定の用途 |
|---|---|
| 実装既定モデル（現在: Claude Opus 5） | マイルストーン級実装・長い自律実行・セキュリティ関連コード（redaction / allowlist / MCP ガード）。**orchestration.md §2 の実行規律 G1〜G6 を必ず適用** |
| 実装既定モデル + 高 effort / 実装前 Plan・Review | migration 設計のような不可逆で難度の高い設計（実装前に独立レビューを 1 本挟む） |
...
- 不変条件 1〜9 への違反がないか（特に書き込み allowlist と read-only 制約）
- 健康データ境界（不変条件 9）— 実健康データがフィクスチャ・ログ・コミット・PR に
  混入していないか。health ストアの独立性と health MCP 既定無効が保たれているか
...
- 依頼範囲の逸脱がないか（求められていない抽象化・リファクタ・「ついで」の変更、
  および黙って範囲を狭めた形跡）
...
- **見つけたものは全部出す**。重要度や確信度でこの段階では絞らない。確信が持てない
  ものも出したうえで [blocker/should/nit] と確信度を添え、取捨は人間が行う。
  「重大なものだけ報告」といった指示はレビュアーが文字どおり従い、実際のバグを
  黙って落とすため使わない。
- 指摘 = 修正ではない。レビュアーはコードを変更しない（orchestration.md §4.2）。
